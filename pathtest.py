...
DATA = [
    {"matrix": [[1, 2, 3], [4, 5, 6], [7, 8, 9]], "expected_path_cost": 2 + 3 + 6 + 9},
    {"matrix": [[1, 2, 5], [4, 3, 8], [7, 7, 9]], "expected_path_cost": 2 + 3 + 7 + 9},
    {
        "matrix": [[3, 1, 2, 5], [2, 5, 4, 1], [4, 2, 9, 6], [4, 3, 2, 4]],
        "expected_path_cost": 1 + 5 + 2 + 3 + 2 + 4,
    },
    {
        "matrix": [[3, 1, 2, 5], [2, 5, 3, 1], [4, 2, 1, 6], [1, 3, 2, 4]],
        "expected_path_cost": 1 + 2 + 3 + 1 + 2 + 4,
    },
    {
        "matrix": [[10, 5, 8, 3], [20, 7, 1, 12], [4, 9, 12, 21], [13, 14, 15, 16]],
...
def matrix_to_edges(matrix):
    ...
    for x in range(w - 1):
        for y in range(h):
            for i in range(2):
                edges.append(
                    [f"x{x}_y{y}_r{i}", f"x{x + 1}_y{y}_r{i + 1}", matrix[x + 1][y]]
                )
            for i in range(3):
                edges.append([f"x{x}_y{y}_u{i}", f"x{x + 1}_y{y}_r0", matrix[x + 1][y]])
                edges.append([f"x{x}_y{y}_d{i}", f"x{x + 1}_y{y}_r0", matrix[x + 1][y]])
    for x in range(w):
        for y in range(h - 1):
            for i in range(2):
                edges.append(
                    [f"x{x}_y{y}_d{i}", f"x{x}_y{y + 1}_d{i + 1}", matrix[x][y + 1]]
                )
            for i in range(3):
                edges.append([f"x{x}_y{y}_r{i}", f"x{x}_y{y + 1}_d0", matrix[x][y + 1]])
                edges.append([f"x{x}_y{y}_l{i}", f"x{x}_y{y + 1}_d0", matrix[x][y + 1]])
    for x in range(1, w):
        for y in range(h):
            for i in range(2):
                edges.append(
                    [f"x{x}_y{y}_l{i}", f"x{x - 1}_y{y}_l{i + 1}", matrix[x - 1][y]]
                )
            for i in range(3):
                edges.append([f"x{x}_y{y}_u{i}", f"x{x - 1}_y{y}_l0", matrix[x - 1][y]])
                edges.append([f"x{x}_y{y}_d{i}", f"x{x - 1}_y{y}_l0", matrix[x - 1][y]])
    for x in range(w):
        for y in range(1, h):
            for i in range(2):
                edges.append(
                    [f"x{x}_y{y}_u{i}", f"x{x}_y{y - 1}_u{i + 1}", matrix[x][y - 1]]
                )
            for i in range(3):
                edges.append([f"x{x}_y{y}_r{i}", f"x{x}_y{y - 1}_u0", matrix[x][y - 1]])
                edges.append([f"x{x}_y{y}_l{i}", f"x{x}_y{y - 1}_u0", matrix[x][y - 1]])
